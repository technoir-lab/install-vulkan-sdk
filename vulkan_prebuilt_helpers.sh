...
function _install_windows_qt() {
  ...
  ./vulkan_sdk.exe --root "$VULKAN_SDK_DIR" --accept-licenses --default-answer --confirm-command install
}
# older SDK installers could be reliably extracteed via 7z.exe
function _install_windows_7z() {
  test -d $VULKAN_SDK_DIR && test -f vulkan_sdk.exe
  echo "Using 7z to unpack Vulkan SDK installer headlessly to $VULKAN_SDK_DIR..." >&2
  7z x vulkan_sdk.exe -aoa -o$VULKAN_SDK_DIR
}
...
function install_mac() {
  ...
  sudo ${InstallVulkan}.app/Contents/MacOS/${InstallVulkan} --root "$sdk_temp" --accept-licenses --default-answer --confirm-command install com.lunarg.vulkan.vma com.lunarg.vulkan.volk com.lunarg.vulkan.ios com.lunarg.vulkan.kosmic
  du -hs $sdk_temp
