...
function install_linux() {
  ...
  if [[ ! -e "$sdk_root/lib/libvulkan.so" && -f "$sdk_root/lib/VulkanLoader/lib/libvulkan.so" ]] ; then
    echo "note: symlinking Vulkan loader from lib/VulkanLoader/lib into lib/ for CMake compatibility" >&2
    ln -s VulkanLoader/lib/libvulkan.so "$sdk_root/lib/libvulkan.so"
    ln -s VulkanLoader/lib/libvulkan.so.1 "$sdk_root/lib/libvulkan.so.1"
  fi
}
